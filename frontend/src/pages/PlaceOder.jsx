...
<div className="mt-8">
  ...
  <div className="mt-12">
    ...
    <div className="flex flex-col lg:flex-row gap-4 mt-6">
      {/* Stripe */}
      <div onClick={()=>setMethod('stripe')} className={`flex items-center gap-3 border p-4 px-5 rounded-md cursor-pointer hover:shadow-md transition `}>
        <p className={`w-4 h-4 border rounded-full  ${method === 'stripe' ? 'bg-green-400' : ''}`}></p>
        <img src={assets.stripe_logo} className="h-6" alt="Stripe Logo" />
      </div>

      {/* Razorpay */}
      <div onClick={()=>setMethod('razorpay')} className={`flex items-center gap-3 border p-4 px-5 rounded-md cursor-pointer hover:shadow-md transition`}>
        <p className={`w-4 h-4 border rounded-full ${method === 'razorpay' ? 'bg-green-400' : ''}`}></p>
        <img src={assets.razorpay_logo} className="h-6" alt="Razorpay Logo" />
      </div>
      <div onClick={()=>setMethod('cod')} className={`flex items-center gap-3 border p-4 px-5 rounded-md cursor-pointer hover:shadow-md transition`}>
